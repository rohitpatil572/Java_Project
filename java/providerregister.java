...
public class providerregister extends HttpServlet {
   ...
   @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)throws ServletException, IOException {
       ...
       {
       String delete=db.Query("delete from provider_register where no='"+no+"'", "Record Deleted");
       out.println(delete);
       resp.setContentType("text/html;charset=UTF-8");
                    out.println("<script type=\"text/javascript\">");
                    out.println("alert(' Account deleted successfully');");
                    out.println("location='provider_register_lst.jsp';");
                    out.println("</script>");
       }
         if(submit.equals("Update"))
       {
           
           String update=db.Query("update provider_register set fname='"+fname+"',lname='"+lname+"',address='"+address+"',number='"+number+"' where no='"+no+"'", "Record Updated");
           out.println(update);
           resp.setContentType("text/html;charset=UTF-8");
                    out.println("<script type=\"text/javascript\">");
